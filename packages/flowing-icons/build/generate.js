...
const entryCode = svgDataFlat
  .map((com) => syntaxEs(com.componentName))
  .join('\n')
const entryCodeCjs = svgDataFlat
  .map((com) => syntaxCjs(com.componentName))
  .join('\n')
...
fs.outputFile(
  '../index.js',
  babel.transform(entryCodeCjs, babelConfigCjs).code,
  (err) => {
    if (err) return
    ...
    nunjucks.render(
      ...
      (err, res) => {
        ...
        const code = babel.transform(
          res,
          cjs ? babelConfigCjs : babelConfig
        ).code
        fs.outputFile(
