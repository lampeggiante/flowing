...
const locale = {
  'zh-CN': {
    ...
    desc2: (
      <span>
        单个组件的话可以直接将以上样式写到 <code>IconXXX</code> 的{' '}
        <code>style</code> 中
      </span>
    ),
    'show-config': '显示配置',
  ...
  'en-US': {
    ...
    desc2: (
      <span>
        For a single component, you can directly write the above style to the{' '}
        <code>style</code>
...
export default function ({ lang = 'zh-CN' }) {
  ...
  const [iconClassifyModalVisible, setIconClassifyModalVisible] =
    useState(false)
          ...
          <div className={className} key={key}>
            ...
            <ul className="iconlist">
              {filteredData.map((n) => {
                ...
                const newIcon = newIcons.find(
                  (_n) => _n.name === n.componentName
                )
                return (
                  <li
                    key={n.componentName}
